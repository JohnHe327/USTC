...
void clearFringe(FRINGE *pFringe)
{
    FRINGE *p;
    for (p = pFringe->pnext; p != NULL; p = pFringe->pnext)
    {
        pFringe->pnext = p->pnext;
        free(p->pNode->act);
        free(p->pNode);
        free(p);
    }
...
NODE *IDAstarSolve()
{
    int d_limit = getHeuristic(input, goal);
    int next_d_limit = 0;
    NODE *min = NULL;
    while (d_limit < INT_MAX)
    {
        ...
        while (!fringeEmpty())
        {
            ...
            if (min->f > d_limit)
            {
                next_d_limit = (next_d_limit > min->f) ? min->f : next_d_limit;
            }
            else
            {
                if (min->h == 0) // bingo!
